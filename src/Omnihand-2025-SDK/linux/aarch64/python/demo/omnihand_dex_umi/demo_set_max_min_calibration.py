...
"""
OmniHand Dex UMI min/max position calibration demo

This demo sets min/max position calibration for OmniHand Dex UMI.
Calibration sets min/max range for position sensing.

Steps:
1. Move fingers to minimum (fully bent), then call set_min_position_calibration()
2. Move fingers to maximum (fully extended), then call set_max_position_calibration()

Notes:
- Calibration writes device parameters
- Perform calibration in a safe environment
- Re-init device after calibration for it to take effect
"""
...
def main():
    ...
    # Prompt user to continue min calibration
    print("=" * 60)
    ...
    # Prompt user to continue max calibration
    print("=" * 60)
    print("WARNING: This will modify device calibration parameters!")
    print("=" * 60)
