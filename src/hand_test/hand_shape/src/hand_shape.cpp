...
static constexpr std::array<double, 10> ACTIVE_JOINT_LOWER = {
  -1.12, -0.05, -0.8416, 0.0, 0.0, 0.0, -0.17, 0.0, -0.19, 0.0,
};
static constexpr std::array<double, 10> ACTIVE_JOINT_UPPER = {
...
private:
  ...
  std::pair<bool, std::string> execute_shape(const std::string & shape_name)
  {
    ...
    for (size_t i = 0; i < ACTIVE_JOINT_NAMES.size(); ++i) {
      ...
      if (!std::isfinite(rad) ||
        rad < ACTIVE_JOINT_LOWER[i] || rad > ACTIVE_JOINT_UPPER[i])
      {
        RCLCPP_ERROR(
          this->get_logger(),
          "预设 %s 越界: %s=%.6f rad，SDK v1.1.8 允许[%.4f, %.4f]",
          shape_name.c_str(), ACTIVE_JOINT_NAMES[i].c_str(), rad,
          ACTIVE_JOINT_LOWER[i], ACTIVE_JOINT_UPPER[i]);
        return {false, "目标超出 SDK v1.1.8 关节限位"};
      }
    ...
    if (exec_result == moveit::core::MoveItErrorCode::SUCCESS) {
      ...
      RCLCPP_ERROR(
        this->get_logger(), "手型 \"%s\" 执行失败 (错误码: %d)",
